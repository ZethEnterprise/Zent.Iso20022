...
function ConvertTo-LEInt
{
    ...
    for ($i = 0; $i -lt 16; $i++)
    {
        $bytes[$i] = [byte][char]$String[$i]
    }

    # Convert bytes to UInt128
    $leInt = [System.UInt128]::Zero
    for ($i = 0; $i -lt 16; $i++)
    {
...
function Test-Iban
{
    ...
    begin
    {
        ...
        {
            Write-Debug "The file '$($script:ibanDataFile)' has not been loaded into the module yet. Loading now..."
    ...
    process
    {
        ...
        # Move first 4 characters to the end for the ISO 13616 checksum algorithm.
